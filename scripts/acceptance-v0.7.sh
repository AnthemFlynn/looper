#!/usr/bin/env bash
# Acceptance test for v0.7 — mcp (RFC).
#
# v0.7 is an RFC milestone, not an implementation milestone. Acceptance is:
...
# (acceptance-v0.8.sh or similar) will replace this one for the
# implementation acceptance.
#
# Run:  ./scripts/acceptance-v0.7.sh
# Or:   make v0.7-acceptance
...
if command -v gh >/dev/null 2>&1; then
  ...
  [ "$STATE" = "CLOSED" ] || \
    fail "issue #9 should be CLOSED when v0.7 RFC is accepted (current state: $STATE)"
